...
def get_log(logger_name):
    ...
    error_log_ptah=os.path.join(path,'logs/error_logs/')
    # 设置日志文件名
    all_log_name=all_log_path+rq+'.log'
    error_log_name=error_log_ptah+rq+'.log'
    # 创建handler
    # 创建一个handler写入所有日志
...
# 最后一行代码是直接在日志类创建了一个实例，括号里面的ccym88可以填项目名称表示哪个项目，在调用日志类
# 的时候，直接import这个log1，不用在使用的时候再去初始化，一来避免重复去初始化，二来可以避免偶尔info日志重复打印的问题
